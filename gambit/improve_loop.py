...
def holdout_pairs(all_items: bool = True) -> list[tuple[Item, BuyerPersona]]:
    """Buyers the optimizer never trains on — for a generalization check."""
    items = ITEMS if all_items else ITEMS[:1]
...
def baseline_strategy() -> Strategy:
    """A deliberately mediocre starting point — a pushover that anchors at list
    and folds fast — so the self-improvement is visible."""
    return Strategy(
        name="gen0",
        gen=0,
...
def improve(generations: int = 6, pairs=None, max_turns: int = 6, verbose: bool = True) -> list[dict]:
    pairs = pairs or default_pairs()
    best = baseline_strategy()
    eps, summ = evaluate(best, pairs, max_turns)
    best_score = summ["score"]
    history = [{"gen": 0, "strategy": best, "summary": summ}]
    if verbose:
        print(f"gen 0  {_fmt(summ)}")
    ...
    for g in range(1, generations + 1):
        ...
        history.append({"gen": g, "strategy": best, "summary": summ})
        if verbose:
            print(f"gen {g}  {_fmt(summ)}")
